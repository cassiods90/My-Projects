...
for(var i=0;i<9;i++) {
	casas[i].addEventListener('click', (event) => {
		...
		if( (event.target.value=='_') && (vencedor=='_')) {
			...
			if (jogador == 'X') {
				event.target.style.color='red'; //torna o valor da casa visível
			} else {
				event.target.style.color='blue'; //torna o valor da casa visível
			}
...
b_reiniciar.addEventListener('click', (event) => {
	for(var i=0;i<9;i++) {
		...
		casas[i].style.color='rgb(251 255 125)'; //Torna o valor _ invisível
		casas[i].style.background='rgb(251 255 125)'; //Torna o fundo branco
	}
...
var sortearJogador = function() {
	if(Math.floor(Math.random() * 2)==0) {
		...
		label_jogador.style.color='blue'; //deixa o texto na cor vermelha
	}else{
		jogador = "X";//define o jogador X como atual
		label_jogador.innerText="X"; //exibe na página qual é o jogador atual
		label_jogador.style.color='red'; //deixa o texto na cor azul
	}
}
